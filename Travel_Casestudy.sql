...
/*Write a query to find no of users per segment who booked flight in april 2022*/
select u.Segment,count(distinct(u.User_id)) as Total_user_count,
count(distinct case when b.Line_of_business='Flight' and (b.Booking_date between '2022-04-01' and '2022-04-30') then b.User_id end) as xyz
from booking_table b join user_table u on b.User_id=u.user_id
group by u.Segment

/*Write a query to identify users whoes first booking was a hotel booking*/
select User_id
from(
select User_id,Booking_date,dense_rank() over(partition by User_id order by Booking_date asc) as rnk,Line_of_business
from booking_table) y 
where rnk=1 and Line_of_business='Hotel'

/*Write a query to find the days between first and lasr booking of each user*/
...
/*Write a query to count the no of flight and hotel bookings in each of the user segement in the year 2022*/

select u.Segment,
count(distinct case when b.Line_of_business='Flight'then b.Booking_id end) as no_of_flight_booking,
count(distinct case when b.Line_of_business='Hotel'then b.Booking_id end) as no_of_hotel_booking
from booking_table b join user_table u on b.User_id=u.user_id
group by u.Segment
